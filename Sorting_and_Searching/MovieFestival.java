...
public class MovieFestival {
    ...
    static void solve(FastReader in, FastWriter out) throws IOException {
        ...
        for(int i = 0 ; i < n ; i ++) {
            timings[i][0] = i;
            timings[i][1] = in.nextInt();
            timings[i][2] = in.nextInt();
        }

        Arrays.sort(timings,Comparator.comparingDouble(o -> o[2]));
        ...
        for(int i = 0 ; i < n ; i++){
            currStart = timings[i][1];

            if(currStart >= lastWatched)  {    
                ans++;
                lastWatched = timings[i][2];
            }
        }
    ...
    static void prinArr(FastWriter out,int arr[][]) throws IOException {
